...
Tags on foods come from two sources:
1. **Semantic categories** — assigned by the LLM during abstraction (e.g. `crackers`, `beverage`, `condiment`)
2. **Computed traits** — generated by `autoTags.ts` from nutrition data (e.g. `high-protein`, `low-sugar`)

The `categories` table classifies every tag with:
- `kind`: `'category'` (food type), `'trait'` (attribute/modifier), or `'unclassified'` (newly minted)
- `parent_slug`: hierarchy pointer (e.g. `corn-chips` → `salty-snacks` → `snack`)

The abstraction pipeline (`jsonStage.ts`) injects the full taxonomy into the LLM prompt.
When the LLM needs a new slug, it must declare it in `new_categories` with kind, parent, and display name.
Undeclared new slugs are registered as `kind='unclassified'` as a safety net.

To classify unclassified tags: `bun run apps/api/src/scripts/classifyTags.ts`
